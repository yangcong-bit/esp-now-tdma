...
typedef struct {
    ...
    uint32_t slot_offset_us;  /**< TX slot offset within the 10 ms TDMA frame (?s). */
...
typedef enum {
    TDMA_PKT_BEACON   = 0xFF, /**< Master ¡ú broadcast: TDMA clock alignment beacon. */
    TDMA_PKT_DATA     = 0x01, /**< Slave  ¡ú master:    user payload container. */
    TDMA_PKT_REG_ACK  = 0x02, /**< Slave  ¡ú master:    join handshake acknowledgement. */
    TDMA_PKT_CMD_OTA  = 0x03, /**< Master ¡ú broadcast: enter OTA mode command. */
    TDMA_PKT_LOG      = 0x04, /**< Slave  ¡ú master:    wireless diagnostic log line. */
} tdma_pkt_type_t;

// ---------------------------------------------------------------------------
// [Packet 0xFF] Master beacon ¡ª synchronisation gun
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// [Packet 0x01] Slave data packet ¡ª user payload container
// ---------------------------------------------------------------------------
typedef struct __attribute__((packed)) {
